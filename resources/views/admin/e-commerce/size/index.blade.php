        ...
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-sm-6 text-right">
                    <a href="{{routeHelper('size/create')}}" class="btn btn-success">
                ...
                <tbody>
                    @foreach ($sizes as $key => $data)
                        <tr>
                            ...
                            <td>
                                <a href="{{ routeHelper('size/'.$data->id.'/edit') }}" class="btn btn-info btn-sm">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="javascript:void(0)" data-id="{{$data->id}}" id="deleteData" class="btn btn-danger btn-sm"">
                                    <i class="nav-icon fas fa-trash-alt"></i>
                                </a>
                                <form id="delete-data-form-{{$data->id}}" action="{{ routeHelper('size/'. $data->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                </form>
